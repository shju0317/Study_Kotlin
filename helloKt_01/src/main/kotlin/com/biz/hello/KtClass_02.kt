...
// 속성 name 과 age에 접근 가능
// 단, name은 val로 선언되어서 읽기전용
...
// 속성에 기본(default)값을 지정하여 객체를 만들 때
// 변수값이 주입되지 않으면 기본값으로 변수를 초기화
// var obj = Class_03("이몽룡", 33) : name 속성과 age를 정의
// var obj = Class_03() : name은 홍길동, age는 0으로 초기화된다.
class Class_03(val name : String = "홍길동", var age : Int = 0){}

// Kotlin은 기본적으로 클래스의 상속이 금지된다.
// 클래스를 상속하려면 클래스 선언자에 open키워드로 시작해야하고
// init블럭이 없어야 한다.
open class Class_04(){
    // 생성자 블럭
    // 클래스로 객체를 생성할 때 무조건 실행되는 블럭
    init {
        println("기본 생성자 블럭")
    }
    ...
    // 상속받은 클래스에서는 반드시 override 키워드로 시작해야 한다
    open fun sum(){
