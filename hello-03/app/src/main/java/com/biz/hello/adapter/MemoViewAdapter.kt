...
class MemoViewAdapter(var context:Context, var memoList : MutableList<MemoVO>) : RecyclerView.Adapter<MemoViewAdapter.MemoHolder?>() {
    ...
    // 자동으로 생성이 되기 때문에 별도로 private변수를 선언하지 않는다.
    // 그리고 이 변수를 클래스 내에서 사용할 때는 null값을 체크할 필요가 없어진다.
    //
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MemoHolder {
        ...
        val view:View = LayoutInflater.from(context).inflate(R.layout.memo_item, parent, false)

        // return new MemoHolder(view) : java
        // MemoHolder클래스에 layout view를 주입하고 객체로 생성하여 return
        return  MemoHolder(view)
    }

    // 생성된 Holder를 list의 item개수만큼 복제하여 Holder들을 생성한다.
    // 이 method는 list의 item 개수만큼 반복 실행되면서 Holder들을 생성하고 RecyclerView에 보여지도록 한다.
    // 단 한번에 실행되는 횟수는 대이터개수가 아닌, 한화면에 보여지는 리스트 개수만큼만 실행되고 기다리니다.
    override fun onBindViewHolder(holder:MemoHolder, position: Int) {

        // holder.txtDate.setDate(memoList.get(position).getDate() <- java
